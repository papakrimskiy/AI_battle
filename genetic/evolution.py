...
class Evolution:
    ...
    def optimize_population(self, team: str):
        # Добавить многоцелевую оптимизацию (NSGA-II)
        population = self.populations[team]
        pareto_front = self._calculate_pareto_front(population)

        # Добавить island model для разнообразия
        islands = self._split_to_islands(population)
